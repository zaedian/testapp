export const translations = {
  nl: {
    loading: {
      title: "Examen Laden...",
      subtitle: "Even geduld a.u.b."
    },
    header: {
      title: "CBR Rijtheorie Oefenen (Categorie B)",
      exitBtn: "Stop",
      readAloudBtn: "Voorlezen",
      stopReadingBtn: "Stop"
    },
    menu: {
      description: "Oefenexamen dat het officiële CBR theorie-examen simuleert.",
      rulesTitle: "Examenopbouw & Eisen",
      selectModeHint: "Selecteer het aantal vragen om door te gaan",
      hazard: "1. Gevaarherkenning",
      knowledge: "2. Kennis",
      insight: "3. Inzicht",
      minCorrect: "Min. {percent}% Goed",
      selectModules: "Selecteer Categorieën",
      selectMode: "Selecteer Examenmodus",
      questionOrder: "Vraagvolgorde",
      randomOrder: "Willekeurig",
      sequentialOrder: "Op Volgorde (vanaf vraag 1)",
      q15: "15 Vragen",
      q30: "30 Vragen",
      q60: "60 Vragen",
      orCustom: "Of eigen aantal:",
      customPlaceholder: "bijv. 25",
      timeLimitTitle: "Tijdslimiet per Vraag",
      timeLimitPlaceholder: "Seconden per vraag",
      secondsUnit: "sec",
      timeLimitHint: "Laat leeg om de standaard tijdslimiet van de vraag te gebruiken",
      startBtn: "Start Oefenexamen",
      startBtnDisabled: "Selecteer eerst een modus",
      advanced: "Geavanceerde Opties"
    },
    quiz: {
      category: "Categorie",
      question: "Vraag",
      of: "van",
      backBtn: "Terug",
      nextBtn: "Volgende Vraag",
      finishBtn: "Examen Beëindigen",
      explanation: "Uitleg:",
      timeout: "Tijd Is Om!",
      exitBtn: "Afsluiten",
      readAloudBtn: "Voorlezen",
      stopReadingBtn: "Stop"
    },
    modal: {
      exitTitle: "Examen Afsluiten?",
      exitMessage: "Weet je zeker dat je wilt afsluiten? Je voortgang wordt niet opgeslagen.",
      cancel: "Annuleren",
      confirm: "Afsluiten",
      confirmReset: "Reset",
      resetTitle: "Reset opties naar standaard?",
      resetMessage: "Weet je zeker dat je alle instellingen wilt resetten naar de standaardwaarden?"
    },
    results: {
      title: "Examen Samenvatting",
      passed: "GESLAAGD",
      failed: "GEZAKT",
      category: "Categorie",
      yourScore: "Je Score",
      required: "Minimaal",
      percentage: "Percentage",
      status: "Status",
      strongestCategoryLabel: "Sterkste categorie:",
      weakestCategoryLabel: "Zwakste categorie:",
      spokenSummaryIntro: "Hier is je examenvatting.",
      spokenOverallScore: "Je hebt {correct} van de {total} vragen goed beantwoord.",
      spokenPassedExam: "Dat betekent dat je bent geslaagd voor het examen.",
      spokenFailedExam: "Dat betekent dat je niet bent geslaagd voor het examen.",
      spokenPassRate: "Je slagingspercentage was {rate} procent.",
      spokenCategoryBreakdownIntro: "Hier is de categorieverdeling.",
      spokenCategoryLine: "{category}: je had {correct} van de {total} goed. Je had {required} nodig om te slagen. {result}",
      spokenCategoryPassed: "Je bent voor deze categorie geslaagd.",
      spokenCategoryFailed: "Je bent voor deze categorie niet geslaagd.",
      spokenStrongestCategory: "Je sterkste categorie was {category}.",
      spokenWeakestCategory: "Je zwakste categorie was {category}.",
      spokenPassedAdvice: "Gefeliciteerd. Je voldoet aan alle categorie-eisen.",
      spokenFailedAdvice: "Focus op de zwakkere categorieën voordat je het opnieuw probeert.",
      spokenDetailedIntro: "Gedetailleerde vraaganalyse staat hieronder.",
      menuBtn: "Hoofdmenu",
      restartBtn: "Examen Opnieuw Starten",
      overallStats: "Algemene Statistieken",
      totalQuestions: "Totaal Vragen",
      correctAnswers: "Juiste Antwoorden",
      incorrectAnswers: "Foute Antwoorden",
      examPassRate: "Vereist slagingspercentage",
      yourPassRate: "Jouw slagingspercentage",
      passRate: "Jouw slagingspercentage",
      passedCategories: "Geslaagde categorieën",
      strongestCategory: "Sterkste categorie",
      weakestCategory: "Zwakste categorie",
      timeSpent: "Tijd Besteed",
      detailedBreakdown: "Gedetailleerde Vraaganalyse",
      showAllAnswers: "Toon Alle Antwoorden",
      showWrongOnly: "Toon Alleen Foute Antwoorden",
      question: "Vraag",
      yourAnswer: "Jouw Antwoord",
      correctAnswer: "Juiste Antwoord",
      explanation: "Uitleg",
      correct: "Juist",
      incorrect: "Fout",
      noWrongAnswers: "Gefeliciteerd! Je hebt alle vragen goed beantwoord.",
      improvementTip: "Verbeteringstip: {tip}"
    },
    buttons: {
      lightMode: "Lichte Modus",
      darkMode: "Donkere Modus"
    },
    settings: {
      autoAdvance: "Automatisch doorgaan naar volgende vraag",
      autoAdvanceHint: "Selecteer automatisch de volgende vraag na het beantwoorden",
      feedbackModeTitle: "Feedback Modus",
      practiceMode: "Oefenmodus (Directe Feedback)",
      examMode: "Examenmodus (Geen Feedback)",
      feedbackModeHint: "Examenmodus verbergt antwoorden tot je de quiz hebt voltooid",
      readAloud: "Voorlezen / Tekst-naar-Spraak",
      readAloudHint: "Lees vragen en antwoorden automatisch voor",
      readAloudSpeed: "Snelheid",
      readAloudTest: "Test Voorlezen",
      soundFx: "Geluidseffecten",
      soundFxHint: "Speel geluiden af voor klikken en antwoorden",
      fontSizeTitle: "Lettergrootte",
      fontSizePreview: "Voorbeeld tekst",
      resetDefaults: "Reset opties naar standaard"
    }
  },
  en: {
    loading: {
      title: "Loading Exam...",
      subtitle: "Please wait a moment"
    },
    header: {
      title: "CBR Driving Theory Practice (Category B)",
      exitBtn: "Exit",
      readAloudBtn: "Read aloud",
      stopReadingBtn: "Stop"
    },
    menu: {
      description: "Practice exam simulating the official CBR theory test.",
      rulesTitle: "Exam Breakdown & Requirements",
      selectModeHint: "Please select the number of questions to continue",
      hazard: "1. Hazard Perception",
      knowledge: "2. Traffic Knowledge",
      insight: "3. Traffic Insight",
      minCorrect: "Min. {percent}% Correct",
      selectModules: "Select Categories",
      selectMode: "Select Exam Mode",
      questionOrder: "Question Order",
      randomOrder: "Random",
      sequentialOrder: "Sequential (from question 1)",
      q15: "15 Questions",
      q30: "30 Questions",
      q60: "60 Questions",
      orCustom: "Or custom amount:",
      customPlaceholder: "e.g. 25",
      timeLimitTitle: "Time Limit per Question",
      timeLimitPlaceholder: "Seconds per question",
      secondsUnit: "sec",
      timeLimitHint: "Leave empty to use question's default time limit",
      startBtn: "Start Practice Exam",
      startBtnDisabled: "Select a mode first",
      advanced: "Advanced Options"
    },
    quiz: {
      category: "Category",
      question: "Question",
      of: "of",
      backBtn: "Back",
      nextBtn: "Next Question",
      finishBtn: "Finish Exam",
      explanation: "Explanation:",
      timeout: "Time's Up!",
      exitBtn: "Exit",
      readAloudBtn: "Read aloud",
      stopReadingBtn: "Stop"
    },
    modal: {
      exitTitle: "Exit Exam?",
      exitMessage: "Are you sure you want to exit? Your progress will be lost.",
      cancel: "Cancel",
      confirm: "Exit",
      confirmReset: "Reset",
      resetTitle: "Reset options to default?",
      resetMessage: "Are you sure you want to reset all settings to default values?"
    },
    results: {
      title: "Exam Summary",
      passed: "PASSED",
      failed: "FAILED",
      category: "Category",
      yourScore: "Your Score",
      required: "Min.",
      percentage: "Percentage",
      status: "Status",
      strongestCategoryLabel: "Strongest category:",
      weakestCategoryLabel: "Weakest category:",
      spokenSummaryIntro: "Here is your exam summary.",
      spokenOverallScore: "You answered {correct} out of {total} questions correctly.",
      spokenPassedExam: "That means you passed the exam.",
      spokenFailedExam: "That means you did not pass the exam.",
      spokenPassRate: "Your pass rate was {rate} percent.",
      spokenCategoryBreakdownIntro: "Here is the category breakdown.",
      spokenCategoryLine: "{category}: you scored {correct} out of {total}. You needed {required} to pass. {result}",
      spokenCategoryPassed: "You passed this category.",
      spokenCategoryFailed: "You did not pass this category.",
      spokenStrongestCategory: "Your strongest category was {category}.",
      spokenWeakestCategory: "Your weakest category was {category}.",
      spokenPassedAdvice: "Congratulations. You met the requirements for every category.",
      spokenFailedAdvice: "Focus on the weaker categories before trying again.",
      spokenDetailedIntro: "Detailed question analysis is shown below.",
      menuBtn: "Main Menu",
      restartBtn: "Restart Exam",
      overallStats: "Overall Statistics",
      totalQuestions: "Total Questions",
      correctAnswers: "Correct Answers",
      incorrectAnswers: "Incorrect Answers",
      examPassRate: "Exam Pass Rate",
      yourPassRate: "Your Pass Rate",
      passRate: "Your Pass Rate",
      passedCategories: "Passed Categories",
      strongestCategory: "Strongest Category",
      weakestCategory: "Weakest Category",
      timeSpent: "Time Spent",
      detailedBreakdown: "Detailed Question Analysis",
      showAllAnswers: "Show All Answers",
      showWrongOnly: "Show Wrong Answers Only",
      question: "Question",
      yourAnswer: "Your Answer",
      correctAnswer: "Correct Answer",
      explanation: "Explanation",
      correct: "Correct",
      incorrect: "Incorrect",
      noWrongAnswers: "Congratulations! You answered all questions correctly.",
      improvementTip: "Improvement Tip: {tip}"
    },
    buttons: {
      lightMode: "Light Mode",
      darkMode: "Dark Mode"
    },
    settings: {
      autoAdvance: "Auto-advance to next question",
      autoAdvanceHint: "Automatically select next question after answering",
      feedbackModeTitle: "Feedback Mode",
      practiceMode: "Practice Mode (Instant Feedback)",
      examMode: "Exam Mode (No Feedback)",
      feedbackModeHint: "Exam mode hides answers until you finish the quiz",
      readAloud: "Enable Read-Aloud / Text-to-Speech",
      readAloudHint: "Read questions and answers aloud automatically",
      readAloudSpeed: "Speed",
      readAloudTest: "Test Read-Aloud",
      soundFx: "Sound Effects",
      soundFxHint: "Play sounds for clicks and answers",
      fontSizeTitle: "Font Size",
      fontSizePreview: "Sample text",
      resetDefaults: "Reset options to default"
    }
  }
};