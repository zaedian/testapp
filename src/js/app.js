...
class ExamApp {
    ...
    _showExitModal() {
        ...
        if (this.readAloudEnabled) this.speakModal(exitModal);
    }
    ...
    showMainMenu() {
        ...
        if (customInput) {
            const t = (window.translations && window.translations[this.currentLang] && window.translations[this.currentLang].menu) || {};
            customInput.placeholder = `${t.totalQuestions || 'Total Questions'}: ${totalQuestions}`;
    ...
    handleOptionSelect(index, isTimeout = false) {
        ...
        if (this.soundFxEnabled && this.feedbackMode === 'practice') {
    ...
    applyFontSize() {
        ...
        if (questionText) questionText.style.fontSize = (this.quizFontSize * 1.25 / 16) + 'rem';
        if (scenarioText) scenarioText.style.fontSize = (this.quizFontSize * 0.95 / 16) + 'rem';
        if (optionsContainer) {
            const optionBtns = optionsContainer.querySelectorAll('.option-btn');
            optionBtns.forEach(btn => {
                btn.style.fontSize = (this.quizFontSize * 0.975 / 16) + 'rem';
            });
        }
        if (answersContainer) {
            const answerCards = answersContainer.querySelectorAll('.answer-card-question');
            answerCards.forEach(card => {
                card.style.fontSize = (this.quizFontSize / 16) + 'rem';
            });
            const answerExplanations = answersContainer.querySelectorAll('.answer-card-explanation');
            answerExplanations.forEach(exp => {
                exp.style.fontSize = (this.quizFontSize * 0.9 / 16) + 'rem';
            });
        }
    }
